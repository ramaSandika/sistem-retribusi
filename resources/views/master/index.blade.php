...
@section('title', 'Master Data Kode Rekening Retribusi')
@section('page_heading', 'Master Data Kode Rekening Retribusi')
...
<div class="card-custom p-3 mb-4">
    <form action="{{ route('master.index') }}" method="GET" class="row g-2 align-items-center">
        <div class="col-md-5 col-12">
            <div class="input-group input-group-sm">
                <span class="input-group-text"><i class="fas fa-search"></i></span>
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari kode rekening, jenis retribusi, kategori..." value="{{ $search }}">
            </div>
        </div>
        ...
        <div class="col-md-3 col-4 d-flex gap-2">
            ...
            @if($search || ($opd && $opd !== 'Semua OPD'))
                <a href="{{ route('master.index') }}" class="btn btn-sm btn-outline-light"><i class="fas fa-times"></i></a>
            @endif
        </div>
    </form>
</div>
...
<div class="card-custom p-4 mb-4">
    <div class="d-flex align-items-center justify-content-between pb-3 mb-3" style="border-bottom: 1px solid rgba(255,255,255,0.12);">
        <span class="fw-bold" style="color:#fca5a5; font-size:0.88rem;">
            <i class="fas fa-database me-2"></i>Daftar Master Rekening ({{ $items->total() }} Data)
        </span>
        <span class="small" style="color:rgba(255,255,255,0.45);">Halaman {{ $items->currentPage() }} dari {{ $items->lastPage() }}</span>
    </div>

    @forelse($items as $idx => $item)
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 py-3"
         style="border-bottom: 1px solid rgba(255,255,255,0.08); {{ $loop->last ? 'border-bottom:none;' : '' }}">
        ...
        <div class="d-flex align-items-center gap-3 flex-grow-1" style="min-width: 280px;">
            ...
            <div>
                ...
                <small style="color:rgba(255,255,255,0.55); font-size:0.78rem;">
                    <i class="fas fa-building me-1 text-danger"></i> {{ $item->opd_name ?: 'Badan Pendapatan Daerah' }}
                </small>
            </div>
        </div>
        ...
        <div class="d-flex align-items-center gap-4 ms-auto">
            <div class="text-end">
                ...
                <span class="fw-bold fs-6" style="color:#86efac;">
                    Rp {{ number_format($item->target_anggaran, 0, ',', '.') }}
                </span>
            </div>
            ...
            <div class="d-flex align-items-center gap-2">
                <button type="button" class="btn btn-sm rounded-circle d-flex align-items-center justify-content-center"
                        ...
                        data-bs-toggle="modal" data-bs-target="#modalEditMaster{{ $item->id }}" title="Edit">
                    <i class="fas fa-pencil-alt" style="font-size:0.80rem;"></i>
                </button>
                <form action="{{ route('master.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus master kode rekening {{ $item->kode_rekening }}?')" class="m-0">
                    @csrf
                    @method('DELETE')
            ...
            <form action="{{ route('master.update', $item->id) }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="kode_rekening" class="form-control fw-bold" value="{{ $item->kode_rekening }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold">Nama Retribusi / Uraian</label>
                        <input type="text" name="nama_retribusi" class="form-control" value="{{ $item->nama_retribusi }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold">Instansi OPD Pengelola</label>
                        <input type="text" name="opd_name" class="form-control" value="{{ $item->opd_name }}">
                    </div>
                    <div class="row g-2 mb-2">
                        ...
                        <div class="col-6">
                            ...
                            <input type="number" name="target_anggaran" class="form-control" value="{{ $item->target_anggaran }}">
                        </div>
                    </div>
                </div>
